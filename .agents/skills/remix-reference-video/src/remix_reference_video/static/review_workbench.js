(() => {
  ...
  const state = { workspace: null, review: null, session: null, selected: null, media: null, preview: null, request: null };
  ...
  const objectRows = (rows, type) => (rows || []).map((row) => {
    ...
    return `<button type="button" class="object-card ${state.selected?.id === id ? "selected" : ""}" data-object-type="${esc(type)}" data-object-id="${esc(id)}">${thumb}<span><span class="object-name">${esc(title)}</span><span class="object-meta">${esc(meta)}</span><span class="object-status ${row.status === "not_ready" ? "not_ready" : ""}">${esc(row.status || "可审核")}</span></span></button>`;
  }).join("");
  function renderRail() {
  ...
  function renderSelection() {
    ...
    renderPreview(ref);
    document.querySelectorAll(".timeline-segment").forEach((segment) => segment.classList.toggle("selected", segment.dataset.objectId === state.selected?.id || segment.dataset.objectId === `timeline-${state.selected?.id}`));
  }
  ...
  function renderTimeline() {
    const timeline = state.workspace.timeline || {}; $("#timeline-source").textContent = timeline.source || "";
  ...
  function renderAssistant() {
    ...
    $("#evidence-list").innerHTML = (decision.evidence || []).map((item) => `<div class="evidence-row ${item.status === "missing" ? "missing" : ""}"><span>${esc(item.artifact)}</span><strong>${item.status === "missing" ? "缺失" : "已具备"}</strong></div>`).join("") || `<div class="empty-rail">暂无</div>`;
  ...
  async function load() { state.workspace = await api("/workspace"); state.review = await api("/review"); await openSession(); renderAssistant(); renderRail(); renderTimeline(); const preview = state.workspace.preview?.media_ref; renderPreview(preview); $("#connection-status").textContent = "已连接"; selectObject("shot", state.workspace.storyboard?.shots?.[0]?.shot_id); connectEvents(); }
  function connectEvents() { const events = new EventSource(`/api/v1/runs/${encodeURIComponent(runId)}/events`); events.addEventListener("revision", (event) => { try { const payload = JSON.parse(event.data); if (Number(payload.state_revision) > Number(state.workspace.state_revision)) location.reload(); } catch (_) {} }); events.onerror = () => { $("#connection-status").textContent = "连接待恢复"; }; }
  ...
  function openChangeDialog() { const options = state.review?.impact_context?.change_options || {}; const types = options.allowed_change_types || ["copy", "material", "voice", "structural"]; $("#change-dialog").classList.add("change-dialog"); $("#change-type").innerHTML = types.map((type) => `<option value="${esc(type)}">${esc({copy:"文案",material:"素材",voice:"音色或语速",structural:"结构"}[type] || type)}</option>`).join(""); renderChangeTargets(); $("#change-dialog").showModal(); }
